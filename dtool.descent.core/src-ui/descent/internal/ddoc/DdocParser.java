...
public class DdocParser {
	...
	private Ddoc internalParse() throws IOException {
		...
		while((line = reader.readLine()) != null) {
			...
			if (colonIndex != -1 && DeeConventions.isValidIdentifier(line.substring(0, colonIndex).trim())) {
				if (currentSectionType == DdocSection.CODE_SECTION) {
					currentSectionType = DdocSection.NORMAL_SECTION;
				}
				...
				if ("Params".equals(currentSectionName) || "Macros".equals(currentSectionName)) { //$NON-NLS-1$ //$NON-NLS-2$
					currentSectionType = "Params".equals(currentSectionName) ? DdocSection.PARAMS_SECTION : DdocSection.MACROS_SECTION; //$NON-NLS-1$
					currentParameterName = null;
	 ...
	 */
	private int getColonOfSectionIndex(String line) {
	...
	private int getEqualsIndex(String line) {
	...
	private void addCurrentSection() {
		if (parameters != null) {
			...
			ddoc.addSection(new DdocSection(currentSectionName, currentSectionType, currentSectionText.toString().trim(), parameters.toArray(new Parameter[parameters.size()])));
		} else {
			ddoc.addSection(new DdocSection(currentSectionName, currentSectionType, currentSectionText.toString().trim()));
		}

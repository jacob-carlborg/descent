...
//	private static final QualifiedName PROJECT_JAVADOC= new QualifiedName(JavaCore.PLUGIN_ID, "project_javadoc_location"); //$NON-NLS-1$
				...
				if (aChild instanceof SourceRefElement) {
					...
					if (start <= position && position <= end) {
						if (child instanceof IField) {
							...
							} while (child != null && child.getSourceRange() != null && child.getSourceRange().getOffset() == declarationStart);
	...
	*/
	/* (non-Javadoc)
	 * @see descent.core.IJavaElement#getSchedulingRule()
	 */
	...
	*/
	...
	protected String getURLContents(String docUrlValue) throws JavaModelException {
		...
		try {
			...
			byte[] contents = descent.internal.compiler.util.Util.getInputStreamAsByteArray(stream, connection.getContentLength());
			if (encoding == null) {

...
public class ClassDeclaration extends AggregateDeclaration {
	...
	public void semantic(Scope sc, SemanticContext context) {
		...
		// printf("ClassDeclaration::semantic(%s), type = %p, sizeok = %d, this
		...
		if (baseclasses.size() > 0) {
			...
			if (tb.ty != TY.Tclass) {
				...
				if (tb.ty != TY.Terror) {
					context.acceptProblem(Problem.newSemanticTypeError("Base type must be class or interface", IProblem.BaseTypeMustBeClassOrInterface, 0, b.sourceType.start, b.sourceType.length));
				}
				...
				if (tc.sym.isInterfaceDeclaration() != null)
					...
					for (ClassDeclaration cdb = tc.sym; cdb != null; cdb = cdb.baseClass) {
						if (cdb == this) {
							...
							context.acceptProblem(Problem.newSemanticTypeError("Circular inheritance", IProblem.CircularDefinition, 0, firstBaseClass.sourceType.start, firstBaseClass.sourceType.length));
							baseclasses.remove(0);
							// goto L7;
							gotoL7 = true;
							break;
		...
		for (i = (baseClass != null ? 1 : 0); i < baseclasses.size();) {
			...
			if (tc == null || tc.sym.isInterfaceDeclaration() == null) {
				...
				for (int j = (baseClass != null ? 1 : 0); j < i; j++) {
					...
					if (b2.base == tc.sym) {
						context.acceptProblem(Problem.newSemanticTypeError("Duplicated interface " + b.sourceType + " for the type " + this.ident, IProblem.DuplicatedInterfaceInheritance, 0, b.sourceType.start, b.sourceType.length));
					}
				}
		...
		if (baseClass == null && ident.ident != Id.Object) {
			...
			baseclasses.add(0, b);
			/* TODO semantic
			Assert.isTrue(b.type.ty == TY.Tclass);
			tc = (TypeClass) (b.type);
			baseClass = tc.sym;
			Assert.isTrue(baseClass.isInterfaceDeclaration() == null);
			b.base = baseClass;
			*/
		...
		if (baseClass != null) {
			/*
			 * TODO semantic interfaces_dim--; interfaces++; // Copy vtbl[] from
			 * base class vtbl.setDim(baseClass.vtbl.dim); memcpy(vtbl.data,
			 * baseClass.vtbl.data, sizeof(void *) * vtbl.dim); // Inherit
			 * properties from base class com = baseClass.isCOMclass(); isauto =
			 * baseClass.isauto; vthis = baseClass.vthis; } else { // No base
			 * class, so this is the root of the class hierarchy vtbl.setDim(0);
			...
			if (sizeok == 0) {
				...
				/*
				 * If this is a nested class, add the hidden 'this' member which
				 * is a pointer to the enclosing scope.
				 */
				...
				{ // Use the base class's 'this' member
					...
					if ((storage_class & STC.STCstatic) != 0)
						error(
								"static class cannot inherit from nested class %s",
								baseClass.toChars());
			...
			/*
			 * Look for special member functions. They must be in this class,
			 * not in a base class.
			 */
			...
			/*
			 * TODO semantic // Allocate instance of each new interface for (i =
			 * 0; i < vtblInterfaces.dim; i++) { BaseClass *b = (BaseClass
			 * *)vtblInterfaces.data[i]; unsigned thissize = PTRSIZE;
			 * 
			 * alignmember(structalign, thissize, &sc.offset); assert(b.offset ==
			 * 0); b.offset = sc.offset; // Take care of single inheritance
			 * offsets while (b.baseInterfaces_dim) { b = &b.baseInterfaces[0];
			 * b.offset = sc.offset; }
			 * 

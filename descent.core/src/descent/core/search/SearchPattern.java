...
import descent.internal.core.search.matching.InternalSearchPattern;
import descent.internal.core.search.matching.LocalVariablePattern;
...
import descent.internal.core.search.matching.PackageDeclarationPattern;
import descent.internal.core.search.matching.PackageReferencePattern;
...
import descent.internal.core.search.matching.TypeParameterPattern;
import descent.internal.core.search.matching.TypeReferencePattern;
 ...
 * </p>
 * @see #createPattern(descent.core.IJavaElement, int)
		...
		switch(mode) {
				...
				switch (token) {
					case ITerminalSymbols.TokenNameDOT:
						if (declaringType == null) {
							...
							String tokenSource = scanner.getRawTokenSourceAsString();
							declaringType += tokenSource + fieldName;
						}
						fieldName = null;
						break;
					case ITerminalSymbols.TokenNameWHITESPACE:
						if (!(ITerminalSymbols.TokenNameWHITESPACE == lastToken || ITerminalSymbols.TokenNameDOT == lastToken))
							mode = InsideType;
						break;
					default: // all other tokens are considered identifiers (see bug 21763 Problem in Java search [search])
						if (fieldName == null)
							fieldName = scanner.getRawTokenSourceAsString();
						else
							fieldName += scanner.getRawTokenSourceAsString();
				}
				break;
			...
			case InsideType:
				switch (token) {
					...
					default: // all other tokens are considered identifiers (see bug 21763 Problem in Java search [search])
						if (type == null)
							type = scanner.getRawTokenSourceAsString();
						else
							type += scanner.getRawTokenSourceAsString();
				}
				break;
				...
				if (argCount == 0) {
					switch (token) {
						case ITerminalSymbols.TokenNameLESS:
							argCount++;
							if (selector == null || lastToken == ITerminalSymbols.TokenNameDOT) {
								if (typeArgumentsString != null) return null; // invalid syntax
								typeArgumentsString = scanner.getRawTokenSourceAsString();
							...
							declaringType += scanner.getRawTokenSourceAsString();
							selector = null;
							break;
						case ITerminalSymbols.TokenNameDOT:
							if (typeArgumentsString != null) return null; // invalid syntax
							if (declaringType == null) {
								...
								declaringType += scanner.getRawTokenSourceAsString() + selector;
							}
							selector = null;
							break;
						...
						default: // all other tokens are considered identifiers (see bug 21763 Problem in Java search [search])
							if (selector == null)
								selector = scanner.getRawTokenSourceAsString();
							else
								selector += scanner.getRawTokenSourceAsString();
							break;
					}
					...
					switch (token) {
						...
						default: // all other tokens are considered identifiers (see bug 21763 Problem in Java search [search])
							if (parameterType == null)
								parameterType = scanner.getRawTokenSourceAsString();
							else
								parameterType += scanner.getRawTokenSourceAsString();
					}
				} else {
			...
			case InsideReturnType:
				if (argCount == 0) {
					switch (token) {
						...
						default: // all other tokens are considered identifiers (see bug 21763 Problem in Java search [search])
							if (returnType == null)
								returnType = scanner.getRawTokenSourceAsString();
							else
								returnType += scanner.getRawTokenSourceAsString();
					}
				} else {

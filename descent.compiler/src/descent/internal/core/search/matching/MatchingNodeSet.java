...
import descent.internal.compiler.util.HashtableOfLong;
...
public void addPossibleMatch(ASTNode node) {
	...
	long key = (((long) node.start) << 32) + node.start + node.length;
...
protected boolean hasPossibleNodes(int start, int end) {
	...
	for (int i = 0, l = nodes.length; i < l; i++) {
		...
		if (node != null && start <= node.start && node.start + node.length <= end)
			return true;
	}
...
protected ASTNode[] matchingNodes(int start, int end) {
	...
	Util.Comparer comparer = new Util.Comparer() {
		public int compare(Object o1, Object o2) {
			return ((ASTNode) o1).start - ((ASTNode) o2).start;
		}
	};

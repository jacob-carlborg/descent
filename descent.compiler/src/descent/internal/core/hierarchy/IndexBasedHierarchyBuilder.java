...
import descent.internal.core.search.SubTypeSearchJob;
...
private void buildForProject(JavaProject project, ArrayList potentialSubtypes, descent.core.ICompilationUnit[] workingCopies, HashSet localTypes, IProgressMonitor monitor) throws JavaModelException {
	...
	if (openablesLength > 0) {
		...
		descent.core.ICompilationUnit[] unitsToLookInside = null;
		if (inProjectOfFocusType) {
			descent.core.ICompilationUnit unitToLookInside = focusType.getCompilationUnit();
			if (unitToLookInside != null) {
				int wcLength = workingCopies == null ? 0 : workingCopies.length;
				if (wcLength == 0) {
					unitsToLookInside = new descent.core.ICompilationUnit[] {unitToLookInside};
				} else {
					unitsToLookInside = new descent.core.ICompilationUnit[wcLength+1];
...
private void buildFromPotentialSubtypes(String[] allPotentialSubTypes, HashSet localTypes, IProgressMonitor monitor) {
	...
	int wcLength;
	descent.core.ICompilationUnit[] workingCopies = this.hierarchy.workingCopies;
	if (workingCopies != null && (wcLength = workingCopies.length) > 0) {
		String[] newPaths = new String[wcLength];
		for (int i = 0; i < wcLength; i++) {
			descent.core.ICompilationUnit workingCopy = workingCopies[i];
	 ...
	 */
	descent.internal.core.util.Util.sortReverseOrder(allPotentialSubTypes);
	...
	try {
		...
		for (int i = 0; i < length; i++) {
			try {
				...
				Openable handle;
				descent.core.ICompilationUnit workingCopy = (descent.core.ICompilationUnit)wcPaths.get(resourcePath);
				if (workingCopy != null) {

 ...
 * cacheFlags.
 */
public void cacheFlags(IType type, long flags) {
	this.typeFlags.put(type, new Long(flags));
}
/**
...
private static byte[] flagsToBytes(Long flags){
	if(flags != null) {
 ...
 */
public long getCachedFlags(IType type) {
	Long flagObject = (Long) this.typeFlags.get(type);
	if (flagObject != null){
		return flagObject.longValue();
	}
	return -1;
}
...
private boolean isInterface(IType type) {
	long flags = this.getCachedFlags(type);
	if (flags == -1) {
		try {
...
public void store(OutputStream output, IProgressMonitor monitor) throws JavaModelException {
	try {
		...
		for (int i = 0; i < count ; i++) {
			...
			output.write(flagsToBytes((Long)this.typeFlags.get(t)));
			output.write(SEPARATOR4);
			byte info = CLASS;

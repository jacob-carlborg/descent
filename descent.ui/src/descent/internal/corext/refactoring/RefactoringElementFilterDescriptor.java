 ...
 * "descent.ui.internal_refactoringElementFilter" extension point.
 * 
 * @since 3.2
 */
public class RefactoringElementFilterDescriptor implements IPluginContribution {
	...
	private RefactoringElementFilterDescriptor(IConfigurationElement element) {
		fElement= element;
		Assert.isNotNull(getFilterClass(), "An extension for extension-point descent.ui.internal_refactoringElementFilter does not specify a valid class"); //$NON-NLS-1$
		Assert.isNotNull(getId(), "An extension for extension-point descent.ui.internal_refactoringElementFilter does not provide a valid ID"); //$NON-NLS-1$
		fFilterOKToUse= true;
	}

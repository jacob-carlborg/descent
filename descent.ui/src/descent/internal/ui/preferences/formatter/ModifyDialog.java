...
import descent.internal.ui.preferences.formatter.ModifyDialogTabPage.IModificationListener;
...
public abstract class ModifyDialog extends StatusDialog implements IModificationListener {
    ...
    private void doValidate() {
		...
		if (!name.equals(fProfile.getName()) && fProfileManager.containsName(name)) {
			updateStatus(new Status(IStatus.ERROR, JavaUI.ID_PLUGIN, IStatus.ERROR, FormatterMessages.ModifyDialog_Duplicate_Status, null));
			return;
		}
		
		if (fProfile.isBuiltInProfile() || fProfile.isSharedProfile()) {
			updateStatus(new Status(IStatus.INFO, JavaUI.ID_PLUGIN, IStatus.INFO, FormatterMessages.ModifyDialog_NewCreated_Status, null));
			return;
		}
    ...
    private IStatus validateProfileName() {
	    ...
	    if (fProfile.isBuiltInProfile()) {
			if (fProfile.getName().equals(name)) { 
				return new Status(IStatus.ERROR, JavaUI.ID_PLUGIN, IStatus.ERROR, FormatterMessages.ModifyDialog_BuiltIn_Status, null);
			}	
    	}
    	
    	if (fProfile.isSharedProfile()) {
			if (fProfile.getName().equals(name)) { 
				return new Status(IStatus.ERROR, JavaUI.ID_PLUGIN, IStatus.ERROR, FormatterMessages.ModifyDialog_Shared_Status, null);
			}
    	}
		
		if (name.length() == 0) {
			return new Status(IStatus.ERROR, JavaUI.ID_PLUGIN, IStatus.ERROR, FormatterMessages.ModifyDialog_EmptyName_Status, null);
		}

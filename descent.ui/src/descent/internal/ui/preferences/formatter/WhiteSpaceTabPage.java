...
public class WhiteSpaceTabPage extends ModifyDialogTabPage
{
			...
			ICheckStateListener, IDoubleClickListener
	{
		...
		private final String PREF_NODE_KEY = JavaUI.ID_PLUGIN
		...
		public SyntaxComponent()
		{
			...
			fTree = new WhiteSpaceOptions().createTreeBySyntaxElement(fWorkingValues);
		...
		{
			final List<Node> checked = new ArrayList<Node>(100);
		...
		public void restoreSelection()
		{
			...
			if(node != null)
			{
				...
				fTreeViewer.setSelection(new StructuredSelection(new Node[]
				{ node }));
				fLastSelected = node;
			}
		}
		...
		public void doubleClick(DoubleClickEvent event)
		{
			...
			if(selection instanceof IStructuredSelection)
			{
				final Node node = (Node) ((IStructuredSelection) selection)
						.getFirstElement();
				fTreeViewer.setExpandedState(node, !fTreeViewer
	...
	private final class JavaElementComponent implements
			ISelectionChangedListener, ICheckStateListener
	{
		
		private final String PREF_INNER_INDEX = JavaUI.ID_PLUGIN
				+ "formatter_page.white_space.java_view.inner"; //$NON-NLS-1$ 
		private final String PREF_OPTION_INDEX = JavaUI.ID_PLUGIN
				+ "formatter_page.white_space.java_view.option"; //$NON-NLS-1$
		...
		{
			fIndexedNodeList = new ArrayList<Node>();
			fTree = new WhiteSpaceOptions()
					.createTreeByDElement(fWorkingValues);
			WhiteSpaceOptions.makeIndexForNodes(fTree, fIndexedNodeList);
		}
			...
			fInnerViewer.setContentProvider(new ITreeContentProvider()
			{
				...
				public Object[] getChildren(Object parentElement)
				{
					final List<Node> children = ((Node) parentElement)
							.getChildren();
					final List<Node> innerChildren = new ArrayList<Node>();
		...
		private void restoreSelections()
		{
			...
			if(node instanceof InnerNode)
			{
				...
				fInnerViewer.setSelection(new StructuredSelection(new Object[]
				{ node }));
		...
		private void innerViewerChanged(InnerNode selectedNode)
		{
			...
			final List<OptionNode> optionsChildren = new ArrayList<OptionNode>();
			...
			for(final Iterator<OptionNode> iter = optionsChildren.iterator(); iter
					.hasNext();)
			{
	...
	{
		private final String PREF_VIEW_KEY = JavaUI.ID_PLUGIN

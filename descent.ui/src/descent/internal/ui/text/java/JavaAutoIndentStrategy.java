...
public class JavaAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
		...
		*/
		...
		switch (node.getNodeType()) {
			...
			case ASTNode.IF_STATEMENT:
			{
				...
				Statement thenStatement= ifStatement.getThenBody();
				...
				Statement elseStatement= ifStatement.getElseBody();
			...
			{
				Expression expression= node.getNodeType() == ASTNode.WHILE_STATEMENT ? ((WhileStatement) node).getExpression() : ((ForStatement) node).getCondition();
				IRegion expressionRegion= createRegion(expression, info.delta);
	...
	private static IRegion getToken(IDocument document, IRegion scanRegion, int tokenId)  {
		...
		try {
			...
			IScanner scanner= ToolFactory.createScanner(false, false, false, false, ASTProvider.SHARED_AST_LEVEL);

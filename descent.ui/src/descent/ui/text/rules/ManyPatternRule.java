 ...
 */
public class ManyPatternRule implements IPredicateRule {
	 ...
	 *
	 * @param startSequences the pattern's start sequence
	 * @param endSequences the pattern's end sequence, <code>null</code> is a legal value
	 * @param token the token which will be returned on success
	 * @param escapeCharacters any character following this one will be ignored
	 * @param breaksOnEOL indicates whether the end of the line also terminates the pattern
	 */
	public ManyPatternRule(String[] startSequences, String[] endSequences, IToken token, char[] escapeCharacters, boolean[] breaksOnEOL) {
		fStartSequence= CharOperation.stringArrayToCharArray(startSequences);
		fEndSequence= CharOperation.stringArrayToCharArray(endSequences);
		fToken= token;
		fEscapeCharacter= escapeCharacters;
	 ...
	 * @since 2.1
	 */
	public ManyPatternRule(String[] startSequence, String[] endSequence, IToken token, char[] escapeCharacter, boolean[] breaksOnEOL, boolean[] breaksOnEOF) {
	 ...
	 * @since 3.0
	 */
	public ManyPatternRule(String[] startSequence, String[] endSequence, IToken token, char[] escapeCharacter, boolean[] breaksOnEOL, boolean[] breaksOnEOF, boolean[] escapeContinuesLine) {
	...
	protected IToken doEvaluate(ICharacterScanner scanner, boolean resume) {
		...
		if (resume) {
			...
			for (int i = 0; i < fStartSequence.length; i++) {
				if (c == fStartSequence[i][0]) {
					if (sequenceDetected(scanner, fStartSequence[i], false)) {
						lastIndex = i;
						if (endSequenceDetected(scanner))
							return fToken;
					}
	...
	protected boolean endSequenceDetected(ICharacterScanner scanner) {
		...
		while ((c= scanner.read()) != ICharacterScanner.EOF) {
			...
			} else if (fEndSequence.length > 0 && c == fEndSequence[lastIndex][0]) {
				// Check if the specified end sequence has been found.
				if (sequenceDetected(scanner, fEndSequence[lastIndex], true))
					return true;
			} else if (fBreaksOnEOL[lastIndex]) {
		...
		if (fBreaksOnEOF[lastIndex]) return true;
		scanner.unread();
		return false;
	}
